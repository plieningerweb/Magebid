...
class Mbid_Magebid_Model_Ebay_Ebat_Items extends Mage_Core_Model_Abstract
{
	...
	//just to understand ebay
	public function getSellerEvents($from,$to,$current_page=1) {
		//Build request
		...
		//problem: ebay dont like the following two options?!
		//$req->setIncludeWatchCount(false);
		//$req->setNewItemFilter(false);

		//DetailLevel is ignored in GetSellerList
		//$req->setDetailLevel('ReturnAll');
		//$req->setGranularityLevel('Medium');

		$outputSelector = array('ItemArray.Item.SellingStatus.CurrentPrice',
		...
		$req->setOutputSelector(implode(',',$outputSelector));
	...
	public function mappingItem($item)
	{
			...
			$response_array['auction_type'] = $item->ListingType;
			$response_array['auction_name'] = $item->Title;
			if($item->SKU != '')
				$response_array['ebay_sku'] = $item->SKU;
