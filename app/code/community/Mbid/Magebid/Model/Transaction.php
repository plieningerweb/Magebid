...
class Mbid_Magebid_Model_Transaction extends Mage_Core_Model_Abstract
{
	...
	protected function _prepareTransactionData($raw_transaction)
	{
		...
		if (isset($this->_auction_data['ebay_sku'])) $transaction_array['product_sku'] = $this->_auction_data['ebay_sku'];

		//set product id
		if (isset($this->_auction_data['product_id']) && $this->_auction_data['product_id'] > 0)
			$transaction_array['product_id'] = $this->_auction_data['product_id'];
		elseif($transaction_array['product_sku'] != '')
		{
     ...
     * @return boolean
     */
	public function saveOrUpdate($raw_transaction, $tryCreateOrder=true)
	{
		//get ebay_item_id
		...
		//prepare transaction data and assign it
		$data = $this->_prepareTransactionData($raw_transaction);

		//assign product-id changes only if transaction is new
		//reason: if product is already reservated and we change now the product-id, we have some horrible inventory problems
		if(!$new_transaction && isset($data['product_id']))
			unset($data['product_id']);

		$this->addData($data);

		//check for reservation if it is a new transaction and product is set
		if($new_transaction && $this->getProductId() > 0)
			$this->_reserveProduct();

		//save data assigend above
		$this->save();
