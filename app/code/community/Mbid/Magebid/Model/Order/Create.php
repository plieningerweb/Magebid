...
class Mbid_Magebid_Model_Order_Create extends Mage_Adminhtml_Model_Sales_Order_Create
{
     ...
     *
     * order "workflow" (already done in this point):
     * =================
     * import of auctions with title/sku
     * transaction import with mapping:
     *  product_id from auction (product_id) if > 0 OR
     *  product_id by search from the shop (by sku) (sku != '')
     *  product_sku: from auction_detail (ebay_sku)
     *  auction_title: from auction_detail (auction_name)
     *
     * order creation:
     * if product_id > 0: use this product, set only price
     * else: use default product, set title, sku ( != ''), price
     *
     * short description:
     * ==================
     * if the auction was connected to a product, this product is also used for every transaction/order
     * if the auction was not connected and a ebay_sku is available and a product with the same sku is existing, we use this product for the order creation
     * if the auction was not connected and no product was found we use a default product (change name, sku) for the order creation
     *
     * the point, we decide whitch product we use is:
     * 	- creation time of auction db entry (set product_id)
     *  - or: (no product_id) insert new transaction with mapping in Mbid_Magebid_Model_Transaction::_prepareTransactionData
     *
     * we have to decide the product-id so early, because we reserve the product (if order is done later)
     * and if afterwards product-id would change, we have some large inventory problems
     *
    ...
    /**
     * Get a raw list of the quote products (from $this->_transactions)
     *
     * @return array $items
     */
	...
	{
		$items = array();
		//for non existing products in shop
		$items[-1] = array();
		foreach ($this->_transactions as $transaction)
		{
			...
			else
			{
				$items[-1][] = array(
					...
					'single_price' => $transaction->getSinglePrice(),
					'title' => $transaction->getAuctionTitle(),
					'sku' => $transaction->getProductSku(),
				);
			}
	...
	protected function _setQuoteProducts()
	{
		...
		if(count($items) > 0)
		{
			...
			{
				$this->_setQuoteProduct($productId,$item);
			}
		}

		if(count($items_nonExisting) > 0)
		{
			$count = 0;
			foreach ($items_nonExisting as $item)
			{
				$this->_setQuoteProduct($this->_getDefaultProductId(),$item,$count);
				$count++;
			}
		}
	...
	/**
	 * get the default product id if a product is not existing in the shop (read createImportOrder() description)
	 *
	 * @return int
	 */
	protected function _getDefaultProductId()
	{
		//@TODO	TODO	take from configuration or something
		return 1;
	}

	/**
	 * set a quote product for the quote and later for the order
	 * set also name, sku and price
	 *
	 * @param int $productId
	 * @param array $item
	 *
	 * @return $this
	 */
	protected function _setQuoteProduct($productId, $item, $count=false)
	{
		//Set product, sku is saved in table transaction and taken from table auction on creation and NOT from ebay
		$this->_order_item = Mage::getModel('catalog/product')->load($productId);

		//If order_item is a product
		if ($this->_order_item instanceof Mage_Catalog_Model_Product)
		{
			//@TODO TODO why we have to set the stock item sperately? and do not cal $this->_order_item->setStockItem($this->_order_item->getStockItem())
			...
			if(isset($item['title']))
				$this->_order_item->setName($item['title']);

			if($count) {
	            //add unique custom option (does not appear) to avoid merging quote items into one quote item (when same product is used)
	            //otherwise we must use more default products than one
	            $additional = array(
                        'label' => 'magebid',
                        'value' => 'count'.$count
	            	);
		...
		//@TODO	TODO insert else block for missing product -> because error message is just, that its out of stock, but it doesnt exist
