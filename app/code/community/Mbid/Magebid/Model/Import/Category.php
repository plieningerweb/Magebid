...
class Mbid_Magebid_Model_Import_Category extends Mage_Core_Model_Abstract
{
	...
	public function buildTree($selected_cat = 0)
	{
		...
		if ($selected_cat!='' && $selected_cat!=0)
		{
			...
			$cat = $this->_loadCategory($selected_cat);
			if($cat->getId())
			{
				$cat_array = $this->_buildRecursiveTree($selected_cat);
			} else
			{
				$this->_treeErrors[] = Mage::helper('magebid')->__('The selected category does no longer exist. Please select a new one and update the profile.');
			}
		}

		//if $selected_cat is null or not found in db
		if(!isset($cat_array))
		{
			//build tree beginning by the root (eBay)
			//set id to zero, to if selected, field is set to zero and ebay wont use it
			$cat_array = array(
						'text'=>'eBay',
						'id'=>'0',
						'path'=>'1',
	...
	public function getTreeErrors() {
		return $this->_treeErrors;
	}
	...
	protected function _loadCategory($selected_cat) {
